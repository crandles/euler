def problem1():
    ...
    for multiple in multiples: 
        for x in range(1, int(max/multiple)+1): 
            if allMultiples.count(x*multiple)==0:
                if x*multiple<1000:
                    allMultiples.append(x*multiple)
        
    allMultiples.sort()
    return sum(allMultiples)
def problem2():
    #Find the sum of all the even-valued terms in the sequence [Fibonacci]
...
def problem2a():
    #redo of problem 2
    max=4000000
    return sum(filter(lambda x: x%2==0,[i for i in fibonacci(max)]))
def factor(n):
    #Return all factors of a number
    #written for problem3()
    factors=[]
...
def problem4():
    ...
    for num1 in range(999, 99, -1):
        for num2 in range(999, 99, -1):
            if isPalindrome(num1*num2) and num1*num2>largest:
                largest=num1*num2
    return largest
def lcm(a, b):
    #Returns the Lowest Common Multiple of a & b
    #written for problem5()
...
def problem5():
    ...
    return reduce(lambda x, y: lcm(x, y), divisors)   #lcm(..lcm(lcm(1, 2), 3)..)
def problem6():
    #Find the difference between the sum of the squares of the first one hundred
    #natural numbers and the square of the sum. (1+2+..+100)^2-(1^2+2^2+..+100^2)
    hundred=[i for i in range(1, 101)]
    return int(sum(hundred)**2-sum([i*i for i in hundred]))
def problem7():
...
def problem8():
    ...
    consecutive=5
    answer=0;
    for x in range(len(num)-(consecutive-1)):
        temp=[int(i) for i in list(num[x:x+consecutive])]
        tAnswer=reduce(lambda x, y: x * y, temp)
        if tAnswer>answer:
            answer=tAnswer
    return answer
def problem9():
    #There exists exactly one Pythagorean triplet for which a + b + c = 1000.
    #and (a<b<c). Find the product abc. 
    from itertools import combinations
    given=1000
    possible=[i for i in range(1, int(given/2))] #values 1-499, as a+b>c
    possible=[i for i in combinations(possible, 3) if sum(i)==given and i[0]**2+i[1]**2==i[2]**2]
    return [i[0]*i[1]*i[2] for i in possible]
def problem9a():
    #redo of problem 9
    #creates all possible a & b combinations (leaving c values to be calculated)
    from itertools import combinations
    given=1000
    possible=[i for i in range(1, int(given/2)-1)]
    return [[[i[0], i[1], int((i[0]**2+i[1]**2)**0.5)],i[0]*i[1]*int((i[0]**2+i[1]**2)**0.5)] for i in combinations(possible, 2) if sum(i)+(i[0]**2+i[1]**2)**0.5==given]

def problem10(size=2000000):
    #Find the sum of all the primes below two million.
    #my implementation of the Sieve of Eratosthenes
    x=[i for i in range(2, size)]
    p=2
    primes=[2]
    index=0
    while p**2<=size:
        for i in range(p+p-1, size-1, p):
            x[i-1]=0
        index+=1
        while x[index]==0:#skips 0's
            index+=1
        p=x[index]#p=next prime
...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48"""
    ...
    answer=0;
    for x in range(20):
        grid[x]=[int(i) for i in grid[x]]
    ##convert string into array of arrays of ints
    for x in range(20):
        xplus=x+3<20 #if true, can calculate rows straight down
        xminus=x-3>-1 #if true, can calculate rows straight up
        for y in range(20):
            yplus=y+3<20 # if true, can calculate rows to the right
            yminus=y-3>-1 #if true, can calculate rows to the left
            if xplus:
...
def collatz(n):
    #Collatz Problem
    #written for problem14
    if n%2==0:
        return int(n/2)
    else:
        return 3*n+1

def problem14(top=1000000, firstrun=False,maxv=1000000,result={}):
    #The following iterative sequence is defined for the set of positive integers:
    #n  n/2 (n is even)
    #n  3n + 1 (n is odd)
    #Which starting number, under one million, produces the longest chain?
    for x in range(2, top+1): #2...top-1
        if x not in result:
            tempSteps=1
            tempVal=x
            rtemp={}
            while tempVal!=1:
                tempSteps+=1
                tempVal=collatz(tempVal)
                if tempVal in result:
                    tempSteps+=result[tempVal]-1
                    tempVal=1
                else:
                    rtemp[tempVal]=tempSteps-1
            for key in rtemp.keys():
                result[key]=tempSteps-rtemp[key]
            result[x]=tempSteps
    if firstrun:
        return result
    else:
        #return max([ (result[x],x) for x in result])
        newResult={}
        ...
        #return newResult
        return max([ (result[x],x) for x in newResult])
def triGen():
    #Generates triangles in sequence
    # 1st triange is 1 , then 3 (1+2), 6 (1+2+3), 10 (1+2+3+4),..
    #written for problem12
    low=0
    high=1
    while True:
        low=low+high
...
def problem12():
    ...
    triangles=triGen()
    junk=triangles.__next__()
    factors=0
    answer=0
    while factors<=500:
        tempTri=triangles.__next__()
        tempFac=len([i for i in dict.fromkeys(factor(tempTri)).keys()])+2 #factor does not return original number and 1
        if tempFac>factors:
            factors=tempFac
            answer=tempTri
    return answer
...
def problem25():
    ...
    while length<1000:
        step+=1
        length=len(str(fib.__next__()))
    return step
def squaresInOrder(max):
    #Yields 1^1, 2^2, 3^3,..,max^max
    #written for problem 48
    i=1
    while i<=max:
        yield i**i
        i+=1
def problem48():
    ...
    return sum(squaresInOrder(1000))%10**10
...
##04 62 98 27 23 09 70 98 73 93 38 53 60 04 23"""
    ...
    for x in range(len(triangle)):
        triangle[x]=[int(i) for i in triangle[x]]
    for row in range(1, len(triangle)):
        triangle[row][0]+=triangle[row-1][0]
        triangle[row][-1]+=triangle[row-1][-1]
        for value in range(1,len(triangle[row])-1):
            triangle[row][value]+=max(triangle[row-1][value],triangle[row-1][value-1])           
    return max(triangle[-1])
def problem67():
    #Find the maximum total from top to bottom in triangle.txt a 15k text file
    #containing a triangle with one-hundred rows.
    #Used algorithm from problem18()
    triangle="""59
73 41
52 40 09
26 53 06 34
...
23 33 44 81 80 92 93 75 94 88 23 61 39 76 22 03 28 94 32 06 49 65 41 34 18 23 08 47 62 60 03 63 33 13 80 52 31 54 73 43 70 26 16 69 57 87 83 31 03 93 70 81 47 95 77 44 29 68 39 51 56 59 63 07 25 70 07 77 43 53 64 03 94 42 95 39 18 01 66 21 16 97 20 50 90 16 70 10 95 69 29 06 25 61 41 26 15 59 63 35"""
    return problem18(triangle)
def problem21():
    #Evaluate the sum of all the amicable numbers under 10000.
    #d(a)=b and d(b)=a where d(n) returns the sum of proper divisors of n
    answer=0
    knownDivisors={1:0}
    for i in range(2, 10000):
        if not i in knownDivisors:
            knownDivisors[i]=sum(factor(i))+1
            if not knownDivisors[i] in knownDivisors:
                possiblePair=sum(factor(knownDivisors[i]))+1
                knownDivisors[knownDivisors[i]]=possiblePair
            if i==knownDivisors[knownDivisors[i]] and knownDivisors[i]<10000 and i!=knownDivisors[i]:
                answer=answer+knownDivisors[i]+i
    return answer
def problem29():
    #How many distinct terms are in the sequence generated by a^b for 2<=a<=100 and 2<=b<=100?
    from itertools import permutations
    numbers=[i for i in range(2,100+1)]
    numbers=[i[0]**i[1] for i in permutations(numbers, 2)]
    numbers.extend([i**i for i in range(2, 100+1)])
    return len([i for i in dict.fromkeys(numbers).keys()])
def problem22():
    ...
    #list to obtain a name score. What is the total of all the name scores in the file?
    ...
    for i in range(len(names)):
        names[i]=(i+1)*sum(map(lambda x:ord(x)-64,names[i]))        
    return sum(names)
def problem42():
    #Using words.txt (right click and 'Save Link/Target As...'), a 16K text file
    #containing nearly two-thousand common English words, how many are triangle words?
    triangles=triGen()
    triangle=triangles.__next__()
    alltriangles=[]
    ...
    for i in range(len(words)):
        words[i]=sum(map(lambda x:ord(x)-64,words[i]))
    while triangle<max(words)+1:
        if triangle in words:
            answer+=words.count(triangle)
        alltriangles.append(triangle)
        triangle=triangles.__next__()
    return answer
def problem40():
    #What is the product of the 1st, 10th, 100th, 1000th, 10000th, 100000th, and 1000000th digits of
    #the fractional part of the irrational fraction: 0.1234567891011..
    counter=0
    digits=0
    ...
    while len(needed)>0:
        ...
        digits+=len(counterS)
        if digits>=needed[0]:
            temp=needed[0]-(digits-len(counterS))
            answer*=int(counterS[temp-1])
            needed.pop(0)
    return answer
def primeGen():
    #Generates prime numbers
    #written for poblem 39
    primes=[]
    counter=1
    while 1:
        counter+=1
        filt=[i for i in filter(lambda x: counter%x==0, primes)]
        if filt==[]:
...
def problem37():
    ...
    ##skip to 11, single digit primes don't count
...
def problem15():
    ...
    size=20
    default=[0 for i in range(size+1)]
    default[-1]=1
    grid=[default[:] for i in range(size+1)]
    grid[-1]=[1 for i in range(size+1)]
...
def problem17():
    ...
    odds={'1':'eleven','2':'twelve','3':'thirteen','4':'fourteen','5':'fifteen','6':'sixteen','7':'seventeen','8':'eighteen','9':'nineteen'}
    words=""
    for i in range(999):
        stringed="%d" % (i+1)
        #print(stringed)
        currentString=""
        if len(stringed)>2:
            currentString+=digits[stringed[0]]+"hundred"
            if stringed[1]!='0' or stringed[2]!='0':
                currentString+="and"
            stringed=stringed[1:]
        ...
        if len(stringed)>1:
            if stringed[0]=='1':
                if stringed[1]=='0':
                    ...
                    currentString+=odds[stringed[1]]
            else:
                currentString+=tens[stringed[0]]
                if stringed[1]!='0':
                    currentString+=""+digits[stringed[1]]
        else:
            currentString=digits[stringed[0]]
        currentString+=""
        words+=currentString
    words+="onethousand"
    return len(words)
def problem19():
    ...
    for i in range(1901, 2001):
        if i%4==0:
            ...
            monthDays[1]=28
        for j in monthDays:
            if start==0: #0 corresponds to Sunday, 1 to Monday, etc..
                sundays+=1;
            start=(start+j)%7
    return sundays
def problem23():
    ...
    for i in range(1,28123):
        if i<(sum(set(factor(i)))+1): #cast as set to trim down to uniques, add one because it's a proper factor
            abundants.add(i) #if it's abundant, add it to the set
        if not any((i-a in abundants) for a in abundants):
            total+=i #check to see if the difference between i and any abundants is also an abundant
